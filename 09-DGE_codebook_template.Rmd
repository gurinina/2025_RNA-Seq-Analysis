...
data <- read.table("data/Mov10_full_counts.txt", header = TRUE, row.names = 1, sep= "\t")
...
X <- model.matrix(~ sampletype, colData(dds))
...
To build the results table, we use the `results()` function. You can specify the `contrast` to be tested using the contrast argument. In this example, we’ll save the unshrunken and shrunken results of **Control vs. Mov10 overexpression** to different variables. 
```{r}
# define contrasts
...
cnetplot(pwt_filtered, showCategory = 15,
         categorySize = "pvalue",
         colorEdge = TRUE) +
  theme(legend.position = "none")
