...
First we create a DESeqDataSet as we did in Chapter \@ref(chap:normalization).
lesson and specify the location of our raw counts and metadata, and input our design formula:
...
To normalize the count data, DESeq2 calculates size factors for each sample using the *median of ratios method* discussed previously in the Chapter \@ref(chap:normalization) lesson. 
...
This curve is displayed as a red line in the figure below, which plots the estimate for the **expected dispersion value for genes of a given expression strength**. Each black dot represents one gene, showing both its mean expression level (x-axis position) and its initial dispersion estimate (y-axis position). Blue dots show the final shrunken estimates that DESeq2 uses for statistical testing. 
