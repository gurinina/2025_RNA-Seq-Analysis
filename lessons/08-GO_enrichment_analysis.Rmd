...
solutions(list(
  quote({
    # Extract log2 fold changes from knockdown results
  ...
  quote({
    # Create input matrix using cbind
    matkd <- cbind(kdfoldchanges, kdfoldchanges)
  }),
  quote({
    # Compute score matrix with significance threshold of 0.58
    scoreMatkd <- compSCORE(matkd, coln = 1, sig = 0.58)
  }),
  quote({
    # Run GO enrichment analysis with FDR threshold of 0.2
    kresp <- runGORESP(fdrThresh = 0.2,
                       bp_input = hGOBP.gmt,
                       go_input = NULL,
                       minSetSize = 20,
                       maxSetSize = 300,
