...
solutions(list(
  quote(res_tableKD <- readRDS("data/res_tableKD.RDS")),
  quote(res_tableKD_tb <- res_tableKD %>% 
    data.frame() %>% rownames_to_column(var = "gene") %>% 
    dplyr::filter(!is.na(log2FoldChange)) %>% as_tibble()),
  quote(kdfoldchanges <- res_tableKD_tb$log2FoldChange),
  quote(names(kdfoldchanges) <- res_tableKD_tb$gene),
  quote(kdfoldchanges <- sort(kdfoldchanges, decreasing = TRUE)),
  quote(matkd <- cbind(kdfoldchanges, kdfoldchanges)),
  quote(scoreMatkd <- compSCORE(matkd, coln = 1, sig = 0.58)),
  quote(kresp <- runGORESP(fdrThresh = 0.2, bp_input = hGOBP.gmt, go_input = NULL, minSetSize = 20, maxSetSize = 300, scoreMat = scoreMatkd))
))
...
solutions(list(
  quote(kvis <- visSetup(kresp$enrichInfo, kresp$edgeMat)),
  quote(runNetwork(kvis$nodes, kvis$edges, main = "MOV10 Knockdown GO Enrichment Network"))
))
