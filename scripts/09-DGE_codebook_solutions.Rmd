...
```{r, results = "asis", echo = FALSE}
solutions(list(quote({
  cat("Yes, sci8_rep2, ctrl0_rep3 cluster away from their replicates")
})))
```
...
```{r, results = "asis", echo = FALSE}
solutions(list(quote({
  cat("Yes, sci8 and sci16 don't separate well, the ctrl0 group shows big variations among replicates")
})))
```
...
solutions(list(
  ...
  quote({
    cat("Explanation of changes:\n")
    cat("- color: Changes the color palette to blue tones\n")
    cat("- border_color: Adds black borders around each cell\n") 
    cat("- fontsize: Sets the overall font size to 10\n")
    cat("- fontsize_row: Sets row name font size to 12\n")
    cat("- show_rownames: Shows the row names (sample names)\n")
    cat("- show_colnames: Shows the column names (sample names)\n")
  })
))
```
...
```{r, results = "asis", echo = FALSE}
solutions(list(quote({
  cat("sampletype, you could also use MOVexpr, but it is redundant with sampletype. Sampletype is also more descriptive.")
})))
```
...
```{r, results = "asis", echo = FALSE}
solutions(list(quote({
  cat("Add the factor as an additional column in the metadata")
})))
```
...
solutions(list(
  quote({
    # Calculate correlation between total counts and size factors
    correlation <- cor(colSums(counts(dds, normalized = FALSE)), sizeFactors(dds))
    correlation
  }),
  quote({
    cat("The total number of raw counts per sample and the size factors are highly correlated.")
    cat("\nThis makes sense because size factors are calculated to normalize for sequencing depth differences.")
  })
))
```
...
```{r, results = "asis", echo = FALSE}
solutions(list(quote({
  cat("Yes, our data appear to be a good fit for the model. The shrinkage we observe is expected and beneficial with small sample sizes. The shrinkage helps stabilize log2 fold change estimates for genes with high dispersion or low counts, making the results more reliable. This is a feature of the DESeq2 model, not a problem with our data.")
})))
```
...
solutions(list(
  quote({
    # Extract beta coefficients from the DESeq2 object
    beta_coefficients <- coef(dds)
  }),
  quote({
  ...
  quote({
    # Calculate log2 fitted values using matrix multiplication
    log2_fitted <- beta_coefficients %*% t(X)
  }),
  quote({
    # Convert from log2 scale to count scale
    fitted_counts <- 2^log2_fitted
  }),
  quote({
    # Adjust for size factors to get normalized fitted counts
    size_factors <- sizeFactors(dds)
    normalized_fitted_counts <- sweep(fitted_counts, 2, size_factors, "*")
  }),
  quote({
    # Compare with DESeq2's internal fitted values
...
solutions(list(
  quote({
    # Number of significant genes with both FDR and fold change criteria
    nrow(sigOE)
  }),
  quote({
  ...
  quote({
    ...
    percent_reduction <- 100 - (nrow(sigOE)/nrow(res_tableOE) * 100)
    percent_reduction
  }),
  quote({
    cat("Yes, adding the fold change threshold reduces the results significantly.")
    cat("\nFrom", nrow(res_tableOE), "total genes to", nrow(sigOE), "significant genes")
    cat("\nThis represents about", round((nrow(sigOE)/nrow(res_tableOE) * 100), 1), "% of the genes")
  })
))
```
...
**Exercise/Homework:** First save a copy of **`lessons/07-DGE_summarizing_workflow.Rmd`** in your student_notes folder as <lastname>_07-DGE_summarizing_workflow.Rmd. Then modify the file to analyze the MOV dataset, starting with Mov10_full_counts.txt in your data folder. Compare the "MOV10_knockdown" to the "control". Include a heatmap and a volcano plot**. points = +10
