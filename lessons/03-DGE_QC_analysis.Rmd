...
**Why this matters:**
Without transformation, highly expressed genes would dominate the PCA and clustering simply because they have larger count values, masking biologically meaningful patterns from other genes.
...
![Alt text](img/simplePCAplot.png){ width=600 }
Analysis Questions

**1. What are the main sources of variation in this dataset (PC1 and PC2)?**
```{r results='asis', echo=FALSE}

solution3("**Answer:** The main sources of variation are the drug treatments. PC1 (38% variance) captures the largest difference - primarily separating Control from Treatment B. PC2 (24% variance) captures the second largest difference - separating Treatment A from Treatment C. Together, these show that different treatments produce distinct gene expression patterns.")
...
solution3("**Answer:** Most groups show good separation and tight replicate clustering (Control, Treatment A, Treatment B). However, Treatment C is problematic - the replicates don't cluster together, with TreatmentC_rep3 positioned far from the other two replicates near Treatment B samples. This suggests either a sample handling issue or high variability in this treatment.")
...
solution3("**Answer:** First, investigate TreatmentC_rep3 as a potential outlier - check for mislabeling or sample swap, and consider excluding it. Second, Treatment C shows high variability overall, which may indicate inconsistent drug effects. Finally, PC1 and PC2 only explain 62% of total variance, so examine PC3 and PC4 to ensure no important patterns are missed.")
```
...
Before creating PCA plots and other visualizations, we need to transform our normalized counts using the regularized log transformation (rlog).

**Why transform the data?**
Raw RNA-seq count data has a problem: genes with low expression show high variability (noise), while highly expressed genes show less variability. This means highly expressed genes would dominate any clustering or PCA analysis.
The rlog transformation solves this by:

Converting counts to log2 scale (makes genes more comparable)
Stabilizing variance across all expression levels (so all genes contribute more equally)
...
solutions(list(
  quote({
    ...
    cat("- fontsize_row: Sets rownames font size to 12\n")
    cat("- fontsize_col: Sets colnames font size to 10\n")
