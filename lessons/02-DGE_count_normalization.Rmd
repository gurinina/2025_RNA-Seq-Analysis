...
solutions(list(
  quote({
    ...
    # Calculate normalized counts by dividing raw counts by size factors
    normalized_counts <- PD1 / size_factors
    normalized_counts
  })
))
```
...
solutions(list(
  quote(idx <- match(rownames(metadata), colnames(counts))),
  quote(counts <- counts[, idx]),
  quote(all(colnames(counts) == rownames(metadata)))
))
