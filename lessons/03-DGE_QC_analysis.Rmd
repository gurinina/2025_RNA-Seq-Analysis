...
solutions(list(
  quote("The sample groups 'Ctrl' and 'Sci' separate well on PC1 and PC2")
))
```
...
solutions(list(
  quote("Yes, sci8_rep2, ctrl0_rep3 cluster away from their replicates")
))
...
solutions(list(
  quote("Yes, sci8 and sci16 don't separate well, the ctrl0 group shows big variations among replicates")
))
...
solutions(list(
  quote(display.brewer.all()),
  quote({
    # This changes the color palette to Blues
    heat.colors <- brewer.pal(9, "Blues")
  }),
  quote({
    # This adds border color, changes font sizes, and shows row/column names
    pheatmap(rld_cor, color = heat.colors, 
             border_color = "black", fontsize = 10, 
             fontsize_row = 12, show_rownames = T,
             show_colnames = T)
  })
))
